frappe.provide("frappe.views.cm_page");

window.cur_list = null;
frappe.views.CmpageFactory = class CmpageFactory extends frappe.views.Factory {
	make(route) {
		const me = this;
		...
		frappe.provide("frappe.views.cm_page." + doctype);
		const hide_sidebar = view_class.no_sidebar || !frappe.boot.desk_settings.list_sidebar;

		frappe.views.cm_page[me.page_name] = new view_class({
			doctype: doctype,
	...
	// before_show() {
	// 	// BT:: Check this function is now required or not.
